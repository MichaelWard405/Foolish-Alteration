...
class FoolishDeployer:
    ...
    def __init__(self, root_window):
        ...
        self.available_auto_starts = self.get_all_items_in_dir(LOCAL_AUTO_STARTS_DIR)
        ...
        self.selected_auto_start = tk.StringVar(value=self.get_default(self.available_auto_starts))
    ...
    def sync_warehouse_to_local(self):
        ...
        try:
            ...
            keywords = {
                ...
                "auto_starts": LOCAL_AUTO_STARTS_DIR,
    ...
    def execute_deployment(self):
        try:
            ...
            if theme_nvim_dir and theme_nvim_dir.exists():
                ...
                # Ensure LazyVim theme plugin directory exists
                lazy_plugins_dir = NVIM_SYS_DIR / "lua" / "plugins"
                lazy_plugins_dir.mkdir(parents=True, exist_ok=True)

                # Auto-detect custom colorscheme name from the colors/ directory (e.g. 'fools_gaze')
                theme_colors_dir = theme_nvim_dir / "colors"
                colorscheme_name = "tokyonight" # Default fallback

                if theme_colors_dir.exists():
                    for color_file in theme_colors_dir.glob("*.lua"):
                        colorscheme_name = color_file.stem
                        break

                # Write/override lua/plugins/theme.lua with the LazyVim colorscheme override
                lazy_theme_config = f"""-- Auto-generated by Foolish Installer
return {{
  {{
    "LazyVim/LazyVim",
    opts = {{
      colorscheme = "{colorscheme_name}",
                ...
                (lazy_plugins_dir / "theme.lua").write_text(lazy_theme_config)
            ...
            if theme_plymouth_dir and theme_plymouth_dir.exists():
                custom_plymouth_name = f"Foolish-{self.selected_theme.get()}"
                sys_plymouth_dest = Path("/usr/share/plymouth/themes") / custom_plymouth_name
                try:
                    subprocess.run(
                        f"sudo mkdir -p {sys_plymouth_dest} && sudo cp -r {theme_plymouth_dir}/* {sys_plymouth_dest}/ && sudo plymouth-set-default-theme -R {custom_plymouth_name}",
                        shell=True, check=False
                    )
                except Exception as ply_err:
                    print(f"Plymouth boot theme provision skipped: {ply_err}")

            #[SCRIPTS & WALLPAPER] [F]
            # Scripts Directory Sync
